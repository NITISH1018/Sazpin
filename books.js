...
async function run()
{
    ...
    const findOneQuery = { author: "Miguel Grinberg" };
    try {
        const findOneResult = await collection.findOne(findOneQuery);
        if (findOneResult === null) {
          console.log("Couldn't find any books that contain 'Miguel Grinberg'.\n");
        } else {
          console.log(`Found a book with 'Miguel Grinberg':\n${JSON.stringify(findOneResult)}\n`);
        }
      } catch (err) {
  ...
  // find one query 
      const findoneQuery1 = { id: "682507" };
    try {
        const findoneResult = await collection.findOne(findoneQuery1);
        if (findoneResult === null) {
          console.log("Couldn't find any 'id: 682507'.\n");
        } else {
          console.log(`Found a book with 'id: 682507':\n${JSON.stringify(findoneResult)}\n`);
        }
      } catch (err) {
        console.error(`Something went wrong trying to find one document: ${err}\n`);
      }
...
// findquery
const findQuery = {  author: "Miguel Grinberg"  };

  try {
    const cursor = await collection.find(findQuery);
    await cursor.forEach(books => {
      console.log(`${books.author} has founded from the books.`);
    });
    // add a linebreak
    console.log();
  } catch (err) {
